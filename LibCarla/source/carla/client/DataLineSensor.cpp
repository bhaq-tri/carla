...
#include "carla/sensor/data/DataLineSensorEvent.h"
...
namespace client {
  ...
  // ===========================================================================
  // -- DataLineSensorCallback ---------------------------------------------------
  // ===========================================================================

  class DataLineSensorCallback {
  public:
  ...
  void DataLineSensorCallback::Tick(const WorldSnapshot &snapshot) const {
    ...
    if (!crossed_lanes.empty()) {
      _callback(MakeShared<sensor::data::DataLineSensorEvent>(
  ...
  std::shared_ptr<const DataLineSensorCallback::Bounds> DataLineSensorCallback::MakeBounds(
      const size_t frame,
  ...
  void DataLineSensor::UpdateSplineData(std::vector<int> &data)
  {
    // Copy the data.
    log_info("DataLineSensor::UpdateSplineData - Trying to Update spline:");
    GetEpisode().Lock()->UpdateSplineData(*this, data);
  }

  void DataLineSensor::UpdateRawData(int &data)
  {
    // Copy the data.
    log_info("DataLineSensor::UpdateRawData - Trying to UpdateRawData: ", data);
    GetEpisode().Lock()->UpdateRawData(*this, data);
  }

  DataLineSensor::~DataLineSensor() {
    Stop();
  }

  void DataLineSensor::Listen(CallbackFunctionType callback) {
    ...
    auto cb = std::make_shared<DataLineSensorCallback>(
        *vehicle,
    ...
    const size_t callback_id = episode->RegisterOnTickEvent([cb=std::move(cb)](const auto &snapshot) {
      try {
        ...
        log_error("DataLineSensor:", e.what());
      }
    });
